#!/usr/bin/perl
# データファイルはExcelでCSVを読み込んでからテキストファイル（タブ区切り.txt）に変換しておく
...
while(my $line = <>) {
	...
	next unless($pid =~ /(\d+)-([0-9PQRS])-(\d+)/o);
	my($day, $room_id, $presen_num) = ($1, $2, $3);
	my $session_key = $room_id;
	
	next unless($time =~ /(\d+):(\d+)[^\d]+(\d+):(\d+)/o);
	
	my($st_h, $st_m, $en_h, $en_m) = ($1,$2,$3,$4);
	
	my $data_day = $data[$day-1];
	my $data_session;
	unless(exists $data_day->{$session_key}) {
		$data_day->{$session_key} = [];
	}
	$data_session = $data_day->{$session_key};

	push @{$data_session}, {
		id => $pid, title => $title,
		authors => $authors, page => $page,
#		time => $time
		start => join(":", $st_h, $st_m),
		end   => join(":", $en_h, $en_m),
	};
